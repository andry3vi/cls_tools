...
def main():
    ...
    run_number = 6213
    # with asdf.open(os.path.join(data_directory, f"run_{run_number}.asdf"), copy_arrays=True) as af:
    #     af.info()
    ...
    TOF_GATE = [54,64]
    PMT_GATE = [3,4] # we have up to 4 pmt on the line depending on the run. For this particular run we used only PMT 3 and 4
    ...
    data.Compute_Voltages() #compute absolute beam energies from the cooler voltage and scanning voltage using the calibration of the voltage dividers
    data.Compute_WL(Mass=Rh103_mass,ref=Frequency,harmonic=2) #compute the particle restframe Frequency from their energy and the laser setpoint. The reference frequency is subtracted
